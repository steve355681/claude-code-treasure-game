...
export default function App() {
  ...
  useEffect(() => {
    if (!gameEnded || mode !== 'user' || !token) return;
    fetch('/api/scores', {
      method: 'POST',
      headers: {
      ...
      .then(res => { if (res.ok) setScoreSaved(true); })
      .catch(() => {});
  }, [gameEnded]); // only fires when game ends; score/token are stable at that point
    ...
    setBoxes(prevBoxes => {
      const updatedBoxes = prevBoxes.map(box => {
        if (box.id === boxId && !box.isOpen) {
          const newScore = box.hasTreasure ? score + 150 : score - 80;
          setScore(newScore);
    ...
    <div className="relative min-h-screen bg-gradient-to-b from-amber-50 to-amber-100 flex flex-col items-center justify-center p-8">
      {mode === 'user' && (
        <div className="absolute top-4 right-4 flex items-center gap-3">
          <span className="text-amber-800 text-sm">👤 {user?.username}</span>
          <Button
            variant="outline"
            className="border-amber-400 text-amber-800 hover:bg-amber-200 text-sm px-3 py-1 h-auto"
            onClick={logout}
          >
            Logout
          </Button>
              ...
              <div className="mb-4 p-6 bg-amber-200/80 backdrop-blur-sm rounded-xl shadow-lg border-2 border-amber-400">
                ...
                {mode === 'user' && (
                  <p className="text-sm mt-2">
                    {scoreSaved
                      ? <span className="text-green-600 font-medium">✓ Score saved!</span>
                      : <span className="text-amber-500">Saving score...</span>
                    }
                  </p>
                )}
                {mode === 'guest' && (
